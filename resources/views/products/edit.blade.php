...
<div class="max-w-3xl mx-auto py-8">
    <h1 class="text-3xl font-bold mb-6 text-amber-700">Edit Produk</h1>
    ...
    <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6 bg-white p-6 rounded-lg shadow-md">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            @if($product->foto)
                <img src="{{ asset('storage/' . $product->foto) }}" alt="{{ $product->name }}" class="mt-2 w-32 h-32 object-cover rounded shadow-sm" />
            @endif
        </div>
